  ...
  fr = fopen ("lista.txt", "rw");
    ...
    while( opt != 0 ) {
        ...
        switch( opt ) {
            ...
            case 1:
                scanf("%d %d", &item.nome, &item.valor);
            ...
            case 2:
                ...
                scanf("%40[^' '] %d", &item.nome, &item.valor);
                strcpy(search->item.nome, item.nome);
            ...
            case 4:
                ...
                printf("%d %d\n", search->item.nome, search->item.valor);
            break;
    ...
    if (!vaziaLista(&lista)){
        ...
        while (paux != NULL) {
            fprintf(fw, "%d %d\n", paux->item.nome, paux->item.valor);
            paux = paux->next;
        }
    }
...
NO *buscar(LISTA_LIGADA *lista, ITEM *item) {
    ...
    while( search != NULL ) {
// printf("Into Buscar...\n" );
        if( strcpy(search->item.nome, item->nome) ) {
            printf("Into Buscar...\n" );
            return search;
        }
        search = search->next;
